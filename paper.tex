...
Facebook's zstd project \cite{zstd1.3.7} has a build that exhibits this property.  When we used our tool to analyze
...
\begin{Verbatim}[commandchars=\\\{\},codes={\catcode`$=3\catcode`^=7\catcode`_=8},fontsize=\small]
  cc \$(FLAGS) common/debug.c \textrm{\emph{29 more .c files}}
    -o libzstd.so.1.3.7
\end{Verbatim}
\caption{A long running cc command which was responsible for approximately 100\% of zstd's span when the command in fig. \ref{code:cc1} is removed.
